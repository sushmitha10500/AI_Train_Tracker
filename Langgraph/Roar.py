...
def find_station_with_ai_and_validation(location: str, is_code: bool = False) -> Optional[Dict]:
    ...
    system_prompt = """
You are a STRICT validation and mapping system for Indian Railways.

THIS APPLICATION SUPPORTS INDIAN RAILWAY STATIONS ONLY.

ABSOLUTE, NON-NEGOTIABLE RULES:
1. Indian Railways operates ONLY within the country of India.
2. If the input location is NOT in India, you MUST:
   - NOT guess
   - NOT approximate
   - NOT substitute with any Indian city
   - NOT suggest nearby or famous Indian places
   - NOT return a station code
3. Foreign countries, cities, regions, continents, landmarks, or airports
   are STRICTLY INVALID.
4. If the input is NOT in India, STOP immediately.

FOR ANY NON-INDIAN LOCATION (examples: Tokyo, China, USA, London, Paris, Europe, Asia):
RETURN EXACTLY THIS JSON AND NOTHING ELSE:

{
  "error": "outside_india",
  "confidence": "high",
  "explanation": "This application supports Indian Railway stations only."
}

ONLY IF THE LOCATION IS INSIDE INDIA:
- Convert it to the EXACT official Indian Railway station code (2–5 uppercase letters)
- Use ONLY real Indian Railway station codes
- You MAY suggest nearest Indian stations if relevant
...
"""
    ...
    try:
        ...
        if not result or result == "AI service unavailable.":
            return None

        result = result.strip().replace("```", "")
        parsed = json.loads(result)

        # 🚫 HARD BLOCK FOR FOREIGN LOCATIONS
        if parsed.get("error") == "outside_india":
            logger.warning(f"🌍 Outside India location blocked: {location}")
            return None

        code = parsed.get("code", "").upper().strip()
        name = parsed.get("name", "").strip()
        confidence = parsed.get("confidence", "medium")
        nearest_stations = parsed.get("nearest_stations", [])
        explanation = parsed.get("explanation", "")

        if code and validate_station_code(code) and validate_station_with_api(code):
